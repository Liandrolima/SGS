...
import { Button, Typography, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, CircularProgress, TextField } from "@mui/material";
...
const Dashboard = () => {
  ...
  const handleSaveEdit = async () => {
    if (!editingResource) {
      console.error("Erro: Nenhum recurso selecionado para edição.");
      return;
    }
  ...
  const handleSaveNewResource = async () => {
    ...
    console.log("📤 Enviando novo recurso:", JSON.stringify(newResource));

    try {
        const addedResource = await api.addResource(newResource);
        if (!addedResource) throw new Error("Erro ao adicionar recurso.");
    ...
    <Paper sx={{ padding: 2, margin: "20px", textAlign: "center" }}>
      ...
      {editingResource && (
        <Paper sx={{ padding: 2, margin: "20px" }}>
          ...
          <TextField fullWidth label="Nome" value={editingResource.name} onChange={(e) => setEditingResource({ ...editingResource, name: e.target.value })} />
          <TextField fullWidth label="Status" value={editingResource.status} onChange={(e) => setEditingResource({ ...editingResource, status: e.target.value })} />
      ...
      {userRole === "admin" && (
        <Paper sx={{ padding: 2, margin: "20px" }}>
          ...
          <TextField fullWidth label="Nome" value={newResource.name} onChange={(e) => setNewResource({ ...newResource, name: e.target.value })} />
          <TextField fullWidth label="Status" value={newResource.status} onChange={(e) => setNewResource({ ...newResource, status: e.target.value })} />
